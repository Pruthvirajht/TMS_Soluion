...
namespace UI_TMS.Controllers
{
    ...
    {
        TmsDatabaseContext db = new TmsDatabaseContext();
        ...
        [HttpPost]
        public IActionResult GenerateReport(string vehicleNo)
        {
            ...
            {
                client.BaseAddress = new Uri("http://localhost:5125");
                var responedata = client.GetAsync("/api/TrafficPolice/GenerateReport/" + vehicleNo);
                responedata.Wait();
                var result = responedata.Result;
                if (result.IsSuccessStatusCode)
                {
        ...
        public IActionResult DeleteOffence(int offenceNo)
        {
            OffenceDetail od = new OffenceDetail();
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri("http://localhost:5125");
                var resp = client.GetAsync("/api/TrafficPolice/Getoffencebyoffno/" + offenceNo);
                resp.Wait();
                var result = resp.Result;
                if (result.IsSuccessStatusCode)
                {
        ...
        [HttpPost]
        public IActionResult DeleteOffence(int offenceNo, OffenceDetail offenceDetail)
        {

            OffenceDetail od = new OffenceDetail();
            offenceNo = offenceDetail.OffenceNo;
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri("http://localhost:5125");
                var resp = client.DeleteAsync("/api/TrafficPolice/DeleteOffenceDeatils/" + offenceNo);
                resp.Wait();
                var result = resp.Result;
                if (result.IsSuccessStatusCode)
                {
        ...
        [HttpPost]
        public IActionResult PayOffence(string vehicleNo)
        {
            vehicleNo = Request.Form["vehicleNo"];
            List<OffenceDetail> offlist = new List<OffenceDetail>();
            using (var client = new HttpClient())
